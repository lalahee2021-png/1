...
r2 = p.add_run(
    ...
    '选取果菜类（番茄、黄瓜）与叶菜类（生菜、小白菜）共4种蔬菜，采用基质袋（槽）培方式，'
...
rows1 = [
    ...
    ('果菜', '黄瓜', '津优35、翠玉',
     '70～85', '9月中下旬育苗',
     '生长适温20～30℃，较耐弱光，12月低温期需保温才能维持结瓜'),
    ('叶菜', '生菜', '意大利生菜、奶油生菜',
     '35～45', '9月下旬起分期播种',
...
rows4 = [
    ...
    ('黄瓜', '55～60℃温汤浸种10～15分钟，或0.1%高锰酸钾溶液浸种20分钟',
     '浸种过程持续搅拌防止局部烫伤种胚，消毒后清水彻底冲洗2～3遍，再浸种4小时使其充分吸水'),
...
sprout = [
    ...
    ('（二）黄瓜', '浸种后同样以湿纱布包裹，置于28～30℃条件下催芽，黄瓜种子发芽适温偏高，'
     '约24～36小时胚根露白（长约0.3～0.5 cm）后播种，避免催芽过长胚根断裂。'),
...
rows6 = [
    ('番茄、黄瓜（果菜通用液）', '950 g', '810 g', '340 g', '500 g', '按说明书剂量加入'),
...
rows6b = [
    ('番茄、黄瓜', '幼苗期', '1.2～1.5', '6.0～6.5', '晴天1～2次/日，基质见干见湿，防止苗期徒长'),
    ('番茄、黄瓜', '开花坐果期', '2.0～2.5', '6.0～6.5', '晴天2～3次/日，每次滴灌至回液率10%～20%'),
    ('番茄、黄瓜', '结果盛期', '2.5～3.0', '6.0～6.5', '晴天3～4次/日，阴雨天减为1次，冬季（12月）适当降低灌溉量'),
...
rows7 = [
    ...
    ('12月', '日均气温15～18℃，早晚最低可至8～10℃，基本无霜冻但夜间偏凉',
     '果菜（番茄、黄瓜）夜间加盖二层或三层大棚膜保温，必要时使用暖风机或电热加温设备；'
     '晴天上午揭膜通风排湿，防止棚温骤升；叶菜耐寒性较强，一般覆盖一层薄膜即可',
     '棚内湿度控制在60%～70%，避免夜间闷湿诱发灰霉病、霜霉病，早晨适当延迟揭膜以防冷凝水滴落伤苗'),
]
...
add_para(doc,
    ...
    '实现叶菜周年不间断供应；番茄、黄瓜9月中下旬育苗，10月中下旬定植，'
    '于12月上中旬陆续始收，采收期可持续至次年1～2月，跨越整个秋冬季生产周期。',
    size=12, first_line=True, space_before=0, space_after=4)
